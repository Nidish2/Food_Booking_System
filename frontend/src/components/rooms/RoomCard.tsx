    ...
    <>
      <article className="rounded-lg border border-brand-border bg-white p-5 shadow-sm">
        ...
        <button 
          ...
          className={`mt-3 flex items-center gap-2 text-sm text-slate-600 ${room.reviews && room.reviews.length > 0 ? 'hover:text-brand-blue cursor-pointer transition-colors' : 'cursor-default'}`}
          aria-label={room.reviews && room.reviews.length > 0 ? "View reviews" : "No reviews available"}
        >
          {averageRating > 0 ? (
            <>
              <StarRating rating={Math.round(averageRating)} />
              <span className="underline decoration-dotted underline-offset-2">{averageRating.toFixed(1)} ({room.reviews?.length} feedback)</span>
            </>
          ) : (
